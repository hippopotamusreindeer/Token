...
contract JERRY is ERC20, ERC20Burnable, Ownable, ReentrancyGuard  {
    ...
    //Testing Variables
    uint256 public amountToSwapETHForJerry;
    uint256 public amountToSwapTokenForToken;
    uint256 public lastEstimatedJerryTokens;
        uint256 public estimatedJerryTokens;
    uint256 public balance_plus_tokens;
   ...
   // Overriding ERC20 transfer to include custom fees and check for max wallet size.
    ...
    function swapTokensForToken(
        ...
        bool checkSize = CheckWalletSizeETHForJerry(to, amountIn);
        require(checkSize, "Swap would exceed maximum wallet balance");
        ...
        amountToSwapTokenForToken = amountToSwap;
    ...
    function swapETHForJerry(
        ...
        amountToSwapETHForJerry = amountToSwap;
    ...
    // Neue Funktion: Schätzt die Menge an Jerry-Token für eine gegebene Menge an ETH
    function getEstimatedJerryForETH(uint256 ethAmount) public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = uniswapRouter.WETH();
        path[1] = address(this); // Adresse Ihres Jerry-Token-Vertrags

        uint256[] memory amounts = uniswapRouter.getAmountsOut(ethAmount, path);
        return amounts[1]; // Geschätzte Menge an Jerry-Token
    }


    //Test boolean if Balance of sender is lower than maximal wallet size
    function CheckBalanceLTMaxWallet () external view returns (bool) {
        return (balanceOf(msg.sender) + amountToSwapTokenForToken) <= _maxWltSize;
    }

    function true_or_falseamountToSwapETHForJerryLTMaxWallet () external view returns (bool){
        return (balanceOf(msg.sender) + amountToSwapETHForJerry) <= _maxWltSize;
    }

    function CheckWalletSizeTokenForToken (address to, uint256 amountIn) internal returns (bool){
        uint256 _maxWltSizeNew = ((totalSupply() * 3) / 100);
        uint256 amountToSwap = _handleFeesAndCalculateAmount(amountIn);
        return (balanceOf(to) + amountToSwap) <= _maxWltSizeNew;
    }

    function CheckWalletSizeETHForJerry (address to, uint256 amountToSwap) internal returns (bool){
        uint256 _maxWltSizeNew = ((totalSupply() * 3) / 100);
        // Calculate the fees
        uint256 burnFeeAmount = _calcBurningFee(msg.value);
        uint256 devFeeAmount = _calcDevFee(msg.value);
        ...
        // Calculate the amount to swap after deducting fees
        amountToSwap = msg.value - burnFeeAmount - devFeeAmount;
        return (balanceOf(to) + amountToSwap) <= _maxWltSizeNew;
    }

    //output der zwischenvariablen
    function Return_AmountToSwap () external view returns (uint256) {
        return amountToSwapETHForJerry;
    }

    function Return_MaxWalletSize () external view returns (uint256) {
        return _maxWltSize;
    }

    function getLastEstimatedJerryTokens () external view returns (uint256) {
        return lastEstimatedJerryTokens;
    }

    function getBalance_plus_Tokens() external view returns (uint256) {
        return balance_plus_tokens;
    }
}
